{ self, inputs, ... }: {
  # Generate Github Matrix
  flake.githubActions = inputs.nix-github-actions.lib.mkGithubMatrix { checks = self.packages; };

  # Build some flake packages that aren't cached anywhere
  flake.packages."x86_64-linux".quickshell-niri = inputs.qml-niri.packages."x86_64-linux".quickshell;
  flake.packages."x86_64-linux".bezel = inputs.bezel.packages."x86_64-linux".default;
  flake.packages."x86_64-linux".lzbt = inputs.lanzaboote.packages."x86_64-linux".lzbt;
  flake.packages."x86_64-linux".stub = inputs.lanzaboote.packages."x86_64-linux".stub;

  flake.packages."aarch64-linux".linux_rpi5 =
    inputs.nixos-raspberrypi.packages."aarch64-linux".linux_rpi5;
  flake.packages."aarch64-linux".raspberrypi-utils =
    inputs.nixos-raspberrypi.packages."aarch64-linux".raspberrypi-utils;
  flake.packages."aarch64-linux".raspberrypi-udev-rules =
    inputs.nixos-raspberrypi.packages."aarch64-linux".raspberrypi-udev-rules;
  flake.packages."aarch64-linux".raspberrypifw =
    inputs.nixos-raspberrypi.packages."aarch64-linux".raspberrypifw;
  flake.packages."aarch64-linux".libraspberrypi =
    inputs.nixos-raspberrypi.packages."aarch64-linux".libraspberrypi;
  flake.packages."aarch64-linux".raspberrypiWirelessFirmware =
    inputs.nixos-raspberrypi.packages."aarch64-linux".raspberrypiWirelessFirmware;
}
